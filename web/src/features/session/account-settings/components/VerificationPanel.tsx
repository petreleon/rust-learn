...
export function VerificationPanel({ session, token }: { session: CurrentSession; token: string | null }) {
  ...
  useEffect(() => {
    if (!token) return;
    void loadAccountKycStatus({ token })
      .then((nextStatus) => {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    setSaveState("saving");
    try {
      const nextStatus = await submitAccountKyc({
        payload: {
    ...
    <article className={styles.panel}>
      ...
      {canSubmit ? (
        <form className={styles.formGrid} onSubmit={handleSubmit}>
          ...
          <label className={styles.fieldLabel}>
            Document last digits
            <input maxLength={16} value={documentLast4} onChange={(event) => setDocumentLast4(event.target.value)} />
          </label>
          <label className={styles.fieldLabel}>
            Evidence reference
            <input
              ...
              placeholder="Optional provider or storage reference"
            />
          </label>
          {message ? <p className={styles.muted}>{message}</p> : null}
          <button className={styles.primaryLink} disabled={saveState === "saving"} type="submit">
            <Send size={17} aria-hidden />
            {saveState === "saving" ? "Submitting" : "Submit KYC"}
          </button>
        </form>
      ) : (
        <p className={styles.muted}>
          <FileCheck2 size={16} aria-hidden />
          {message || "No KYC action is required from this screen right now."}
        </p>
      )}
    </article>
  );
