...
async fn submit_assessment_attempt(
        ...
        .await
    {
        ...
        Err(e) => {
            log::error!("event=assessment_submit_lookup_failed assessment_id={} error={}", assessment_id, e);
...
pub fn course_scope() -> actix_web::Scope {
    web::scope("/courses")
        ...
        .service(web::resource("/{id}/assessments/{assessment_id}/submit").route(web::post().to(submit_assessment_attempt)))
        .service(web::resource("/{id}/assessments/{assessment_id}/attempts").route(web::get().to(list_assessment_attempts)))
        .service(
            web::resource("/{id}")
                .route(
                    web::get()
