...
use crate::application::organizations::list_organization_courses::{
    record_organization_course_reward_queue_status, OrganizationCourseListError,
    OrganizationCourseRewardQueueSummaryOutput, OrganizationCourseRosterSummaryOutput,
};
...
pub async fn load_course_reward_queue_summary(
    ...
    let mut summary = OrganizationCourseRewardQueueSummaryOutput {
    ...
    for (status, count) in load_reward_candidate_status_counts(conn, course_id).await? {
        if let Ok(status) = RewardCandidateStatus::parse(&status) {
            record_organization_course_reward_queue_status(&mut summary, status, count);
        }
    }
    Ok(summary)
}
...
async fn load_reward_candidate_status_counts(
    conn: &mut AsyncPgConnection,
    course_id: i32,
) -> Result<Vec<(String, i64)>, OrganizationCourseListError> {
