...
mod tests {
    ...
    fn normalizes_valid_scopes() {
        assert_eq!(normalize_scope_type(REWARD_FRAUD_BLOCK_SCOPE_TEACHER).unwrap(), "teacher");
        assert_eq!(normalize_scope_type(REWARD_FRAUD_BLOCK_SCOPE_ORGANIZATION).unwrap(), "organization");
        assert_eq!(normalize_scope_type(REWARD_FRAUD_BLOCK_SCOPE_COURSE).unwrap(), "course");
        assert_eq!(normalize_scope_type(REWARD_FRAUD_BLOCK_SCOPE_REWARD_POLICY).unwrap(), "reward_policy");
    }
    ...
    fn course_and_policy_scopes_require_one_permission() {
        for scope in &[REWARD_FRAUD_BLOCK_SCOPE_COURSE, REWARD_FRAUD_BLOCK_SCOPE_REWARD_POLICY] {
