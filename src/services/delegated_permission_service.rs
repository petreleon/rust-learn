...
mod tests {
    ...
    fn normalizes_scope_types() {
        assert_eq!(normalize_scope_type(DELEGATED_SCOPE_PLATFORM).unwrap(), "platform");
        assert_eq!(normalize_scope_type(DELEGATED_SCOPE_ORGANIZATION).unwrap(), "organization");
        assert_eq!(normalize_scope_type(DELEGATED_SCOPE_COURSE).unwrap(), "course");
        assert_eq!(normalize_scope_type("  COURSE  ").unwrap(), "course");
        assert_eq!(normalize_scope_type("Organization").unwrap(), "organization");
    }
    ...
    fn accepts_valid_reward_permissions() {
        let valid = [
            "APPROVE_REWARD_AMOUNT", "EXECUTE_REWARD_PAYOUT", "VIEW_REWARD_AUDIT",
            "MANAGE_REWARD_FRAUD_BLOCKS", "BLOCK_REWARD_TEACHER", "BLOCK_REWARD_ORGANIZATION",
            "SUBMIT_ORG_COURSE_REWARD_EVENT", "VIEW_ORG_REWARD_REPORTS", "MANAGE_ORG_REWARD_BUDGET",
            "SUBMIT_COURSE_REWARD_EVENT", "CREATE_REWARDABLE_COURSE_EVENT",
            "APPROVE_STUDENT_REWARD_CANDIDATE", "VIEW_COURSE_REWARD_STATUS",
            "GRADE_REWARDABLE_ASSESSMENT", "MANAGE_COURSE_REWARD_RULES",
        ];
        for perm in valid {
