...
pub fn assign_role_to_user_in_organization(
    conn: &mut PgConnection,
    assigner_id: i32,
    p_user_id: i32,
    ...
    let assigner_level = RoleOrganizationHierarchy::get_min_level(conn, assigner_id, p_organization_id)?
        .ok_or(diesel::result::Error::NotFound)?; // Assigner must have a role in the org

    // 2. Get Assignee's (Target User) Hierarchy Level
    let assignee_level_opt = RoleOrganizationHierarchy::get_min_level(conn, p_user_id, p_organization_id)?;

    // 3. Get Role ID
    let role_id = crate::models::role::OrganizationRole::find_by_name(role_name, conn)?;

    // 4. Get Target Role's Hierarchy Level
    let target_role_level = RoleOrganizationHierarchy::get_role_level(conn, role_id)?;
    ...
    UserRoleOrganization::assign(conn, p_user_id, p_organization_id, role_id)
}
