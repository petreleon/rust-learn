...
pub fn create_course_with_invites(
    ...
    conn.transaction(|conn| {
        ...
        if let Some(first_org_id) = organization_ids.first() {
            ...
            for org_id in organization_ids.iter().skip(1) {
                create_course_organization_invite(conn, course.id, *org_id)?;
            }
        }
...
pub fn create_course_organization_invite(
    conn: &mut PgConnection,
    course_id: i32,
    organization_id: i32,
) -> QueryResult<usize> {
...
pub fn accept_course_organization_invite(
    conn: &mut PgConnection,
    invite_id: i32,
) -> QueryResult<usize> {
