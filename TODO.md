...
- [ ] **Docker & Deployment**
  ...
  - [x] **Migrate S3 Storage:** Replace `minio` image with `rustfs` in `docker-compose.yml` to avoid AGPLv3 licensing issues. Replace `minio` Rust crate with `aws-sdk-s3`. Rename `MinioState` → `S3State` and `MANAGE_MINIO_OBJECTS` → `MANAGE_S3_OBJECTS`.
