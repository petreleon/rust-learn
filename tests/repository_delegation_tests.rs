...
use rust_learn::models::delegated_permission::{NewDelegatedPermission, DELEGATED_SCOPE_COURSE};
...
async fn test_create_and_find() {
    ...
    let d = delegated_permission_repository::create_delegated_permission(
        &mut conn, new_del(grantor.id(), grantee.id(), "VIEW_COURSE_REWARD_STATUS", 1),
    ).await.unwrap();

    assert_eq!(d.grantor_user_id, grantor.id());
    assert_eq!(d.grantee_user_id, grantee.id());

    let found = delegated_permission_repository::find_delegated_permission(&mut conn, d.id).await.unwrap();
...
async fn test_find_active() {
    ...
    delegated_permission_repository::create_delegated_permission(
        &mut conn, new_del(grantor.id(), grantee.id(), "VIEW_COURSE_REWARD_STATUS", 2),
    ).await.unwrap();

    let active = delegated_permission_repository::find_active_delegated_permission(
        &mut conn, grantee.id(), "VIEW_COURSE_REWARD_STATUS", DELEGATED_SCOPE_COURSE, None, Some(2),
    ).await.unwrap();
    assert!(active.is_some());

    let none = delegated_permission_repository::find_active_delegated_permission(
        &mut conn, grantee.id(), "VIEW_COURSE_REWARD_STATUS", DELEGATED_SCOPE_COURSE, None, Some(999),
    ).await.unwrap();
    assert!(none.is_none());
}
...
async fn test_revoke() {
    ...
    let d = delegated_permission_repository::create_delegated_permission(
        &mut conn, new_del(grantor.id(), grantee.id(), "VIEW_COURSE_REWARD_STATUS", 3),
    ).await.unwrap();

    let revoked = delegated_permission_repository::revoke_delegated_permission(
        &mut conn, d.id, revoker.id(), Some("no longer needed".to_string()),
    ).await.unwrap();
...
async fn test_list_by_grantee() {
    ...
    for i in 0..2 {
        delegated_permission_repository::create_delegated_permission(
            &mut conn, new_del(grantor.id(), grantee.id(), "VIEW_COURSE_REWARD_STATUS", i + 10),
        ).await.unwrap();
    }

    let list = delegated_permission_repository::list_delegated_permissions(
        &mut conn, DelegatedPermissionFilter { grantee_user_id: Some(grantee.id()), ..Default::default() },
    ).await.unwrap();
...
async fn test_active_filter_excludes_revoked() {
    ...
    let d = delegated_permission_repository::create_delegated_permission(
        &mut conn, new_del(grantor.id(), grantee.id(), "VIEW_COURSE_REWARD_STATUS", 100),
    ).await.unwrap();

    let active_before = delegated_permission_repository::list_delegated_permissions(
        &mut conn, DelegatedPermissionFilter { grantee_user_id: Some(grantee.id()), active: Some(true), ..Default::default() },
    ).await.unwrap();
    assert!(active_before.iter().any(|d2| d2.id == d.id));

    delegated_permission_repository::revoke_delegated_permission(&mut conn, d.id, grantor.id(), None).await.unwrap();
...
async fn test_has_active_platform_delegation() {
    ...
    delegated_permission_repository::create_delegated_permission(
        &mut conn, NewDelegatedPermission {
            grantor_user_id: grantor.id(), grantee_user_id: grantee.id(),
            permission: "VIEW_REWARD_AUDIT".to_string(),
            scope_type: "platform".to_string(),
            organization_id: None, course_id: None,
            reason: None, expires_at: None,
        },
    ).await.unwrap();

    let has = delegated_permission_repository::has_active_platform_delegation(
        &mut conn, grantee.id(), "VIEW_REWARD_AUDIT",
    ).await.unwrap();
    assert!(has);
}
