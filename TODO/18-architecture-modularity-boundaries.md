...
- Remaining integration-test harness cleanup: 5 `tests/*/imports.rs` files and
  56 test `include!` calls.
...
| 326 | this batch | Converted `delegated_permissions` to explicit modules/support; proved with focused delegated-permissions tests, Cargo gates, scans showing no local `include!`/`imports.rs`, and line checks. |
...
- Finish global integration-test harness cleanup. Remaining files:
  - `tests/reporting_exports/imports.rs`
  - `tests/reward_candidates/imports.rs`
...
Suggested next small batch: convert `tests/reporting_exports` from
`include!`/`imports.rs` to explicit modules plus `support.rs`, then rerun the
focused reporting-exports test and the standard gates.
