...
Last verified commit before current batch: `a3a7bbbf`.
...
| 334 | `a3a7bbbf` | Typed reward amount decision output status at the infra/application boundary; infra parses persisted candidate status before returning use-case output, HTTP maps it back to the public string, route smoke accepts the current permission-decision middleware error wording, and proof covered amount-decision filters, reward/delegation/API routing integration crates, Cargo gates, scans, and line checks. |
| 335 | this batch | Typed reward candidate submission output status at the infra/application boundary; submission creation and idempotent replay parse persisted candidate status before returning use-case output, HTTP maps it back to the public string, and proof covered submission filters, mapper status tests, reward/delegation/API routing integration crates, Cargo gates, scans, and line checks. |
